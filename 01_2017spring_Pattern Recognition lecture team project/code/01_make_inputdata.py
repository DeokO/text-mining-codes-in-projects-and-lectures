...
economic = pd.read_csv('./data/economic_news_article/economic_news_dataset.csv', delimiter=',', encoding='utf-8')
oshumed = pd.read_csv('./data/ohsumed/ohsumed_dataset.csv', delimiter=',', encoding='utf-8')
reuter = pd.read_csv('./data/reuter/reuters_dataset.csv', delimiter=',', encoding='utf-8')
# newsgroup = pd.read_csv('./data/20newsgroup/20newsgroup_dataset.csv', delimiter=',', encoding='utf-8')
...
def removeStopwords(data):
    ...
    for doc in data:
        ...
        i += 1
    return corpus
economic_stop = removeStopwords(economic_stem)
oshumed_stop = removeStopwords(oshumed_stem)
reuter_stop = removeStopwords(reuter_stem)
...
economic_NN = extract_NN(economic_stop)
oshumed_NN = extract_NN(oshumed_stop)
reuter_NN = extract_NN(reuter_stop)
...
### DTM with tfidf
docu_economic = list(map(lambda x: ' '.join(x), economic_stop))
...
pickle.dump(economic_tfidf_dtm_, open('./data/TOTAL/economic_tf_idf_model_100.pickle', 'wb'))
pickle.dump(economic_tfidf_dtm, open('./data/TOTAL/economic_tf_idf_100.pickle', 'wb'))

docu_oshumed = list(map(lambda x: ' '.join(x), oshumed_stop))
...
pickle.dump(oshumed_tfidf_dtm_, open('./data/TOTAL/oshumed_tf_idf_model_100.pickle', 'wb'))
pickle.dump(oshumed_tfidf_dtm, open('./data/TOTAL/oshumed_tf_idf_100.pickle', 'wb'))

docu_reuter = list(map(lambda x: ' '.join(x), reuter_stop))
...
pickle.dump(reuter_tfidf_dtm_, open('./data/TOTAL/reuter_tf_idf_model_100.pickle', 'wb'))
pickle.dump(reuter_tfidf_dtm, open('./data/TOTAL/reuter_tf_idf_100.pickle', 'wb'))
...
docu_economic = [' '.join(x) for x in economic_stop]
...
docu_oshumed = [' '.join(x) for x in oshumed_stop]
...
docu_reuter = [' '.join(x) for x in reuter_stop]
...
pickle.dump(scaled_economic_p_d_t, open('./data/TOTAL/economic_LDA_model_100.pickle', 'wb'))
...
pickle.dump(scaled_oshumed_p_d_t, open('./data/TOTAL/oshumed_LDA_model_100.pickle', 'wb'))
...
pickle.dump(scaled_reuter_p_d_t, open('./data/TOTAL/reuter_LDA_model_100.pickle', 'wb'))
...
#namedtuple 만드는 과정
economic_tagged_tr_document = [economic_tagged_document(words, [tags]) for tags, words in enumerate(economic_stop)]
...
oshumed_tagged_tr_document = [oshumed_tagged_document(words, [tags]) for tags, words in enumerate(oshumed_stop)]
...
pickle.dump(oshumed_doc_vectorizer, open('./data/TOTAL/oshumed_Doc2Vec_model_100.pickle', 'wb'))

reuter_tagged_document = namedtuple('reuter_tagged_document', ['words', 'tags'])
reuter_tagged_tr_document = [reuter_tagged_document(words, [tags]) for tags, words in enumerate(reuter_stop)]
...
pickle.dump(economic_doc2vec, open('./data/TOTAL/economic_Doc2Vec_model_100.pickle', 'wb'))
oshumed_doc2vec = np.asarray(oshumed_doc_vectorizer.docvecs)
pickle.dump(oshumed_doc2vec, open('./data/TOTAL/oshumed_Doc2Vec_model_100.pickle', 'wb'))
reuter_doc2vec = np.asarray(reuter_doc_vectorizer.docvecs)
pickle.dump(reuter_doc2vec, open('./data/TOTAL/reuter_Doc2Vec_model_100.pickle', 'wb'))
